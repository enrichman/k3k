...
func (c *ClusterReconciler) createCluster(ctx context.Context, cluster *v1alpha1.Cluster, log *zap.SugaredLogger) error {
	...
	var foundSecret v1.Secret
	if err := c.Client.Get(ctx, key, &foundSecret); err == nil {
		// no err, bootstrapSecret already exists

		fmt.Println("bootstrapSecret exists, update if not equal ")
		if !reflect.DeepEqual(foundSecret.Data, bootstrapSecret.Data) {
			foundSecret.Data = bootstrapSecret.Data

			fmt.Println("bootstrapSecret exists, update!! ")
			if err := c.Client.Update(ctx, &foundSecret); err != nil {
				return err
			}
		}
	} else {
