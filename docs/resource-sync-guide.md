...
K3k automatically syncs resources between your virtual clusters and the host cluster, enabling your workloads to run seamlessly while maintaining isolation. This guide explains what resources are synced, why, and how to control sync behavior for your use case.
...
**Workload Resources (Virtual → Host):** Your applications need services, configuration, and secrets to run. These flow from your virtual cluster to the host where your pods actually execute.

**Infrastructure Resources (Host → Virtual):** Storage classes represent available storage infrastructure. These flow from the host into your virtual cluster so your applications can request the right type of storage.
...
- Disabling this will break networking for your workloads - only disable if you know what you're doing
...
**Security:** Synced secrets remain isolated within your cluster's namespace boundary and aren't shared with other virtual clusters.
...
**Why disabled by default:** Not all applications need external access. Enable Ingresses when you need to expose services outside the cluster.

**TLS Secret Translation:** When you create an Ingress in your virtual cluster, any TLS secrets it references are synced from the virtual cluster to the host with translated names (just like other resources). This is the default behavior.
...
**Why disabled by default:** Priority settings affect cluster-wide scheduling and could impact other workloads. Enable only when you need guaranteed scheduling for critical applications.
...
**Why reverse direction:** StorageClasses represent storage infrastructure provided by the host. Your virtual cluster consumes these storage options rather than creating them.
...
1. **Host-Level Label (Optional):** Add `k3k.io/sync-enabled` label to StorageClasses on the host to control sync
...
2. **Cluster-Level Selector (Optional):** Further filter which StorageClasses appear in your virtual cluster based on labels
...
**Policy overrides cluster configuration:** When a policy is applied, its sync settings take precedence over individual cluster settings.
